...
//=============================================================
// 文件名       : demo_interp_dac8_audio_rate_common.v
// 模块名       : demo_interp_dac8_audio_rate_common
// 功能简述     : 正式采样率 DAC 演示公共模块。
//                基于 50MHz 系统时钟和 audio_rate_ce_gen，
//                产生 48kHz / 44.1kHz 家族所需的多级 CE，
//                驱动 interp128_top_ce 插值链，并将 4x / 8x /
//                128x 节点结果映射到 8bit 并行 DAC 输出。
// 设计作者     : kafeizizi 
// 创建日期     : 2026-05-16
// 版本         : V2018.3 
// 开发工具     : Vivado
...
module demo_interp_dac8_audio_rate_common (
    ...
    output wire [1:0]  mode_led       // 当前模式编码，板级顶层可以不用接到真实 LED
);
    ...
    //
    // MODE_128X：
    //   固定输出最终 128x 节点 y_out。
    //
    // RATE_SEL_48K：
    //   1 表示使用 48kHz 家族：
    //     输入 48kHz，4x 192kHz，8x 384kHz，128x 6.144MHz。
    //   0 表示使用 44.1kHz 家族：
    //     输入 44.1kHz，4x 176.4kHz，8x 352.8kHz，128x 5.6448MHz。
    //
    // ROM_ADDR_STEP：
    ...
    // 2）实例化正式采样率 CE 发生器
//
    // audio_rate_ce_gen 负责从 50MHz 系统时钟产生平均频率
    // 精确的 CE 脉冲。
    //
    // 当 RATE_SEL_48K = 1 时：
    //   ce128_out      ≈ 6.144MHz
    ...
    //
    // 注意：
    //   ce128_out / ce8_out / ce4_out 都是单周期 CE 脉冲，
    //   不是 50% 占空比时钟。
    //=========================================================
    wire ce4_out;
    wire ce8_out;
    ...
    //
    // dac_data_r：
    //   保存当前要送给 DAC 的 8bit 数据。
    //
    // dac_clk_r：
    //   输出给 DA9708 的采样脉冲。
    //   现在不再只拉高 1 个 50MHz 时钟周期，
    //   而是拉高 3 个 50MHz 时钟周期。
    //
    // 为什么要拉宽：
    //   原来 1 拍脉冲宽度只有 20ns。
    //   杜邦线连接、探头地线较长、DAC 模块输入端反射，
    //   都可能让示波器把一个脉冲误判成多个尖峰。
    //
    // 数据和时钟的关系：
    //   selected_valid 有效时，先更新 dac_data_r。
    //   下一拍开始输出 dac_clk_r 高脉冲。
    //   这样 DAC 采样边沿到来前，dac_data 已经稳定。
    //=========================================================
    localparam [2:0] DAC_CLK_HIGH_CYCLES = 3'd3;

    reg [7:0] dac_data_r;
    reg       dac_clk_r;
    reg       dac_clk_start_d;
    reg [2:0] dac_clk_high_cnt;

    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            dac_data_r        <= 8'd128;
            dac_clk_r         <= 1'b0;
            dac_clk_start_d   <= 1'b0;
            dac_clk_high_cnt  <= 3'd0;
        end
        else begin
            dac_clk_start_d <= 1'b0;
            ...
            if (selected_valid) begin
                ...
                dac_clk_start_d <= 1'b1;
            end

            if (dac_clk_start_d) begin
                dac_clk_r        <= 1'b1;
                dac_clk_high_cnt <= DAC_CLK_HIGH_CYCLES - 3'd1;
            end
            else if (dac_clk_high_cnt != 3'd0) begin
                dac_clk_r        <= 1'b1;
                dac_clk_high_cnt <= dac_clk_high_cnt - 3'd1;
            end
            else begin
                dac_clk_r <= 1'b0;
            end
