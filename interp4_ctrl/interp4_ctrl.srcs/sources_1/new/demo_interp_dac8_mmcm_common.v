`timescale 1ns / 1ps
//=============================================================
// 文件名       : demo_interp_dac8_mmcm_common.v
// 模块名       : demo_interp_dac8_mmcm_common
...
module demo_interp_dac8_mmcm_common (
    ...
    localparam [5:0] ROM_ADDR_STEP = 6'd1;

    (* mark_debug = "true", keep = "true" *) reg [1:0] mode_state;
    (* mark_debug = "true", keep = "true" *) wire [1:0] ila_mode_sel_w;

    assign ila_mode_sel_w = mode_sel;
    ...
    wire ce64_out;
    wire ce128_out;
    (* mark_debug = "true", keep = "true" *) wire x_in_update_ce;
    ...
    //=========================================================
    (* mark_debug = "true", keep = "true" *) reg signed [23:0] x_in;
    (* mark_debug = "true", keep = "true" *) reg               x_in_valid;
    (* mark_debug = "true", keep = "true" *) reg [5:0]         rom_addr;
    ...
    wire               y_out_valid_w;
    (* mark_debug = "true", keep = "true" *) wire signed [23:0] dbg_y4_w;
    (* mark_debug = "true", keep = "true" *) wire               dbg_y4_valid_w;
    (* mark_debug = "true", keep = "true" *) wire signed [23:0] dbg_y8_w;
    (* mark_debug = "true", keep = "true" *) wire               dbg_y8_valid_w;
    ...
    //
    // mode_sel = 2'b11：
    //   DAC 直接输出原始输入 x_in，用于判断问题是否已经发生在插值链之前。
    //
    // selected_valid 表示当前 selected_sample 是新样本。
    // 后面 DAC 只在 selected_valid 有效时更新数据。
    //=========================================================
    (* mark_debug = "true", keep = "true" *) reg signed [23:0] selected_sample;
    (* mark_debug = "true", keep = "true" *) reg               selected_valid;

    always @(*) begin
        if (mode_sel == 2'b11) begin
            selected_sample = x_in;
            selected_valid  = x_in_update_ce;
        end
        else begin
            case (mode_state)
                MODE_4X: begin
    ...
    (* mark_debug = "true", keep = "true" *) reg        [7:0]  sample_u8_w;
    ...
    //=========================================================
    (* mark_debug = "true", keep = "true" *) reg [7:0] dac_data_r;
    ...
    assign dac_clk = (mode_sel == 2'b11)       ? ce_cnt[6] :
