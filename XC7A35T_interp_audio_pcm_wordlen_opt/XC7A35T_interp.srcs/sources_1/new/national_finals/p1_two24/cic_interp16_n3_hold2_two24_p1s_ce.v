...
// Exact P1-S CIC candidate: C2 -> Hold16 -> one TWO24 DSP integrator pair.
// Compared with cic_interp16_n3_hold2_dsp_ce, the numeric output sequence is
// identical and y_out_valid is delayed by one clk.  No state update or output
// event is added; the registered DSP result drains on the following clk.
module cic_interp16_n3_hold2_two24_p1s_ce #(
    ...
    nf_cic_two24_integrator_pair_p1s u_two24_integrator_pair (
        .clk(clk),
        .rst_n(rst_n),
...
`ifndef SYNTHESIS
    always @(posedge clk) begin
        if (rst_n && x_in_valid && (comb_active || align_pending ||
                                    burst_pending))
            $fatal(1, "P1-S TWO24 CIC input overwrite");
        if (rst_n && BURST_COUNTER_USE_DSP != 0)
            $fatal(1, "P1-S TWO24 CIC only supports a LUT burst counter");
        if (rst_n && FINAL_PRUNE_LSB != 0)
            $fatal(1, "P1-S TWO24 prototype requires FINAL_PRUNE_LSB=0");
        if (rst_n && (DATA_W != 21 || FIRST_INT_W != 26 || FINAL_W != 29))
            $fatal(1, "P1-S TWO24 prototype requires 21/26/29-bit widths");
    end
`endif
endmodule
