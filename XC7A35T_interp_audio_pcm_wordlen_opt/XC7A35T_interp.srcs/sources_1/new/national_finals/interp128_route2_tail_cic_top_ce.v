...
// National-finals Route 2B:
//   FIR2 -> FIR2 -> FIR2 -> sparse HB2 -> sparse HB2 -> CIC4/N2.
//
// The two sparse halfbands use shift/add arithmetic in this first area
// candidate.  DSP count is therefore expected to be five:
//   Stage1=1, shared Stage2/3=1, CIC comb=1, CIC integrators=2.
module interp128_route2_tail_cic_top_ce (
    input  wire                         clk,
    input  wire                         rst_n,
    input  wire                         ce2_out,
    input  wire                         ce4_out,
    ...
    // The two canonical halfbands share their shift/add and saturation
    // datapath because their filtered phases are mutually exclusive.
    interp2_dual_halfband7_shared_shiftadd_ce #(
        .DATA_W(20)
    ) u_route2_shared_halfbands (
        .clk(clk),
        .rst_n(rst_n),
        .ce16_out(ce16_out),
        .ce32_out(ce32_out),
        .x8_in(y8_w),
        .x8_in_valid(y8_valid_w),
        .y16_out(y16_w),
        .y16_out_valid(y16_valid_w),
        .y32_out(y32_w),
        .y32_out_valid(y32_valid_w),
        .phase16_dbg(),
        .phase32_dbg()
    );

    cic_interp4_n2_serial_comb_dsp_ce #(
        .DATA_W(20),
        .OUTPUT_W(20)
    ) u_route2_cic4 (
        .clk(clk),
...
`ifndef SYNTHESIS
    always @(posedge clk) begin
        if (rst_n && ce64_out === 1'bx)
            $fatal(1, "Route2 unused ce64 contains X");
    end
`endif
