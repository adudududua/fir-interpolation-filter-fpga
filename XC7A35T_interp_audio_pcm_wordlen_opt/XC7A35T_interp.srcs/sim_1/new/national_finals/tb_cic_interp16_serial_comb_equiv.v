...
module tb_cic_interp16_serial_comb_equiv;
    ...
    // Buffer reference samples and compare whenever the candidate is valid.
    ...
    initial begin
        ...
        // Deliberately reset while both implementations are emitting a
        // burst. This verifies that no delayed serial-comb state leaks into
        // the next operating epoch.
        stream_inputs(25, 0);
        @(negedge clk);
        x_in_valid = 1'b0;
        ce_out = 1'b1;
        ...
        // Continuous ce_out: boundary values plus deterministic random data.
        stream_inputs(160, 0);
        drain_and_check(160);

        pulse_reset();

        // Random ce_out stalls exercise the enable-domain state transitions.
        stream_inputs(240, 1);
        drain_and_check(240);
