...
module tb_cic_interp16_n3_hold2_two24_p1s_equiv;
    ...
    wire ref_valid;
    wire signed [OUTPUT_W-1:0] y_p1s;
    wire p1s_valid;
    ...
    cic_interp16_n3_hold2_two24_p1s_ce #(
        ...
        .clk(clk), .rst_n(rst_n), .ce_out(ce_out),
        .x_in(x_in), .x_in_valid(x_in_valid),
        .y_out(y_p1s), .y_out_valid(p1s_valid),
        .burst_remaining_dbg(), .pending_dbg(), .comb_busy_dbg()
    );
        ...
        if (!rst_n) begin
            ...
            if (ref_valid) begin
                if (queue_count >= QUEUE_DEPTH)
                    $fatal(1, "P1-S compare queue overflow");
            ...
            if (p1s_valid) begin
                if (queue_count == 0)
                    $fatal(1, "P1-S output arrived without a reference item");
                if (y_p1s !== expected_queue[queue_read])
                    $fatal(1, "P1-S CIC mismatch output=%0d expected=%0d actual=%0d",
                        candidate_count, expected_queue[queue_read], y_p1s);
    ...
    initial begin
        ...
        // Reset in the active burst and therefore in a possible TWO24
        // high-limb pending interval.  The comparison queue is discarded.
        stream_inputs(25, 0);
        @(negedge clk);
        x_in_valid = 1'b0;
        ce_out = 1'b1;
        ...
        $display("P1C TWO24 CIC EQUIVALENCE PASS: continuous=320 stalled=480 seeds=20x256 reset-mid-burst=PASS outputs=%0d 0 LSB",
            candidate_count);
        $finish;
    end
endmodule
