...
> 项目：44.1/48 kHz、24 bit PCM，4×/8×/128×可切换插值滤波器
> 正式实板基线：Vivado 2025.2，239 LUT / 388 FF / 3 DSP48E1 / 4 RAMB18E1（2 BRAM Tile）/ 2 MMCM
> 报告日期：2026-08-15
> 证据原则：数学模型、RTL、实现、网表活动功耗和物理板结论分层记录，不互相替代
...
任何可综合RTL都必须有有限位宽，因此这里不是无限精度，而是：
...
创新边界必须说明：Hogenauer CIC、Noble恒等式和Hold实现思想是已有多速率理论，不能声称发明了新数学公式。项目可主张的创新是：在本双采样率、多倍率音频链中，把恒等式落实为可切换RTL，完成定点模运算与状态位宽证明、DSP/LUT映射、逐拍等价、时序/DRC和板级闭环。[Hogenauer原始CIC论文](https://doi.org/10.1109/TASSP.1981.1163535)和[MathWorks CIC插值说明](https://www.mathworks.com/help/dsp/ref/cicinterpolation.html)均表明CIC及通过Noble恒等式移动速率变换属于既有基础理论。
...
| 物理板 | 用户实测六档采样率和DAC波形 | **通过** |

板测结论只赋予用户明确验证的239-LUT版本；全2×和268-LUT等工具候选不借用该结论。
...
5. **双采样率、动态倍率和板级闭环**：44.1/48 kHz双时钟家族、4×/8×/128×切档、复位/CDC/ROM/DAC均进入17/17 RTL、路由后六模式和用户物理板验证。

## 12. 局限与后续实验

1. 早期结构消融使用Vivado 2018.3，当前架构比较使用2025.2；报告没有跨工具直接归因单颗LUT。
2. SAIF受1 mW分辨率和MMCM主导限制；需要板级电流仪作更细功耗测量。
3. Fmax每种结构只有三档约束和有限布局配方；论文终稿可增加5～10组独立实现配方和均值/标准差。
4. 全2×对照未完成本轮物理板测，只能标为工具闭环。
5. 音频质量还可增加THD+N、SINAD、SFDR和真实音频文件对照，不能用频响替代全部音质。
6. 本次Hold新复跑受16 GB主机内存限制在route阶段终止；报告采用同参数历史签核点，后续可在释放内存后重新完成一次成对Fresh run以增强统计完整性。
...
本报告应与[项目创新点说明报告](项目创新点说明报告.md)配套使用：前者回答做了什么实验、结果是否支持，后者回答创新机制和答辩口径是什么。
