...
- Stage2/Stage3末轮局部调整将S2/S3 pending请求改为两条相互独立的镜像直线，并将PATDET移至PREG上方，使PREG高位检测虚线与`>>>15`实线数据通路并联且互不交叠；最终自然宽约160.8 mm，按16 cm插入时最小字号约8 pt。
- PREG高位检测支路最终从PREG上边框正中垂直引出，经正交折线进入PATDET下边框中点，不再采用边角锚点；该虚线与右侧`>>>15`实线数据通路保持完全分离。
- Stage2/Stage3共享MAC中的乘法节点由三角形固定增益符号改为圆形双输入`×`算子：X从左侧输入、B从下侧输入、乘积M从右侧输出；该表示更准确对应`M=X×B`并与圆形求和节点保持统一。
