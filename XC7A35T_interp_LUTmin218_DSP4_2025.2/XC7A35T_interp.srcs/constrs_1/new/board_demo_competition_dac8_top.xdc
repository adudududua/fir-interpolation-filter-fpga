...
# mode_shadow[1:0] 是捆绑数据 CDC 总线。源域从请求发起开始保持两位数据
# 稳定，直至目的域返回同步确认；目的域随后再等待三个音频时钟周期，才原子锁存
# 两位模式。这里把物理偏斜限制在一个 20 MHz 源时钟周期内，防止布局布线把两位
# 数据分离得过远而破坏握手假设。按照 Vivado 2025.2 的 set_bus_skew 端点规则，
# 约束使用时序单元，而不是寄存器的 Q/D 管脚。
...
# 仅限制相对偏斜不能约束两位数据共同的绝对传输时间。源域会保持 mode_shadow
# 直至收到确认，目的域还会在锁存前等待三个音频时钟周期，因此使用一个 20 MHz
# 控制周期作为绝对数据路径上限，既保守又具有充足的协议裕量。
