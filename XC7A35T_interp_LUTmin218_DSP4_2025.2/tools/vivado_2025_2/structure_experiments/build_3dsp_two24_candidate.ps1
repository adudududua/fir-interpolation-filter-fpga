...
$tclScript = Join-Path $scriptDir 'build_3dsp_two24_candidate.tcl'
...
$resultDir = Join-Path $scriptDir "results\three_dsp_two24_$stamp"
...
$projectHashBefore = (Get-FileHash -Algorithm SHA256 -LiteralPath $projectFile).Hash
...
if ($buildFailed) {
    throw "3-DSP TWO24 candidate failed. See $resultDir\vivado.log"
}

Write-Host 'THREE_DSP_TWO24_POWERSHELL_PASS'
