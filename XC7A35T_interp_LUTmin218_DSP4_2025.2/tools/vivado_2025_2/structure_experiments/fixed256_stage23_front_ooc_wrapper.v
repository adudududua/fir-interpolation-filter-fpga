...
module fixed256_stage23_front_ooc_wrapper (
    ...
    bridge_valid_quantized_to_interp2_ce #(
        ...
        .clk(clk), .rst_n(rst_n), .in_data(y2_raw),
        .in_valid(y2_raw_valid), .ce_out_next(ce4_out),
        .phase_current(stage2_phase), .out_data(y2_to_stage2),
        .out_valid(y2_to_stage2_valid)
    );
    bridge_valid_quantized_to_interp2_ce #(
        .IN_W(22), .OUT_W(20), .SHIFT_N(2), .USE_EXTERNAL_PHASE(1)
    ) u_bridge_4_to_8 (
        .clk(clk), .rst_n(rst_n), .in_data(y4_raw),
        .in_valid(y4_raw_valid), .ce_out_next(ce8_out),
        .phase_current(stage3_phase), .out_data(y4_to_stage3),
        .out_valid(y4_to_stage3_valid)
    );
    interp2_stage23_fixed256_dsp_ce u_stage23 (
        .clk(clk), .rst_n(rst_n),
        .stage2_ce_out(ce4_out), .stage2_x_in(y2_to_stage2),
        .stage2_x_in_valid(y2_to_stage2_valid),
        .stage2_y_out(y4_raw), .stage2_y_out_valid(y4_raw_valid),
        .stage3_ce_out(ce8_out), .stage3_x_in(y4_to_stage3),
