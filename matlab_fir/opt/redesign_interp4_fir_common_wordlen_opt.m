...
%=============================================================
% 文件名       : redesign_interp4_fir_common_wordlen_opt.m
% 脚本名       : redesign_interp4_fir_common_wordlen_opt
% 功能简述     : 4倍插值公共 FIR 的字长优化与稀疏系数裁剪搜索脚本。
%                本脚本在原 redesign_interp4_fir_common.m 的基础上，
...
%
%                输出文件：
%                  interp4_wordlen_sparse_search_result.csv
%                  interp4_wordlen_opt_summary.txt
%                  fir_coeff_decimal_wordlen_opt.txt
%                  fir_coeff_half_decimal_wordlen_opt.txt
%                  fir_coeff_half_for_verilog_wordlen_opt.txt
%                  interp4_wordlen_opt_response.png
%
% 当前默认配置：
%                  输入采样率家族：44.1kHz / 48kHz
%                  输出采样率    ：176.4kHz / 192kHz
...
% 开发工具     : MATLAB
% 修订记录     :
%                2026-06-23：新增 4x FIR 字长优化与稀疏裁剪搜索版本。
%=============================================================
...
% 字长优化候选参数
FRAC_W_LIST    = [16 15 14 13 12];     % 候选小数位宽
PRUNE_THR_LIST = [0 1 2 4 8 16];       % 整数量化系数裁剪阈值

% 最苛刻输出采样率：44.1kHz -> 176.4kHz
Fs_out_worst = 176400;
Fs_in_worst  = Fs_out_worst / 4;

f_pass_low   = 10;
f_pass_high  = 20000;
f_stop_begin = Fs_in_worst - f_pass_high;   % 44100 - 20000 = 24100 Hz
...
% RIPPLE_1764_DB, STOP_1764_DB, RIPPLE_1920_DB, STOP_1920_DB,
...
for frac_w_try = FRAC_W_LIST
    ...
    for prune_thr = PRUNE_THR_LIST
        ...
        for n = n_est : 2 : N_MAX
            ...
            % 3.6 检查 176.4k 和 192k 两种输出模式
            ok_1764 = check_one_mode(b_q, ...
                                      176400, ...
                                      f_pass_low, ...
                                      f_pass_high, ...
                                      stop_attn_target_db, ...
            ...
            pass_all = ok_1764.pass_all && ok_1920.pass_all;
            ...
            result_table = [result_table; ...
                ...
                ok_1764.ripple_pm_db, ok_1764.stop_attn_db, ...
                ok_1920.ripple_pm_db, ok_1920.stop_attn_db, ...
                nonzero_full, nonzero_half, pass_all];

            fprintf('FRAC=%2d COEFF=%2d prune=%2d n=%3d tap=%3d | nz_half=%3d | 176.4k=%d 192k=%d\n', ...
                    frac_w_try, coeff_w_try, prune_thr, n, n+1, ...
                    nonzero_half, ok_1764.pass_all, ok_1920.pass_all);
            ...
            if pass_all
                ...
                if update_best
                    ...
                    best_res_1764     = ok_1764;
                    best_res_1920     = ok_1920;
                end
...
if ~found
    error('未找到满足指标的 4x FIR 字长优化组合。请扩大 N_MAX 或减少裁剪阈值。');
end
...
fprintf('\n================ 最佳 4x FIR 详细检查 ================\n');
res_1764 = check_one_mode_verbose(best_b, ...
                                  176400, ...
                                  f_pass_low, ...
                                  f_pass_high, ...
                                  stop_attn_target_db, ...
...
res_1920 = check_one_mode_verbose(best_b, ...
                                  192000, ...
                                  f_pass_low, ...
                                  f_pass_high, ...
                                  stop_attn_target_db, ...
...
% 5.1 搜索结果 CSV
fid = fopen('interp4_wordlen_sparse_search_result.csv', 'w');
fprintf(fid, 'FRAC_W,COEFF_W,PRUNE_THR,ORDER_N,TAPS,RIPPLE_1764_DB,STOP_1764_DB,RIPPLE_1920_DB,STOP_1920_DB,NONZERO_FULL,NONZERO_HALF,PASS_ALL\n');
...
% 5.2 完整十进制系数
writematrix(best_coeff_int, 'fir_coeff_decimal_wordlen_opt.txt', 'Delimiter', 'tab');

% 5.3 半系数十进制
half_taps       = (length(best_coeff_int) - 1) / 2;
coeff_half_int  = best_coeff_int(1:half_taps + 1);
writematrix(coeff_half_int, 'fir_coeff_half_decimal_wordlen_opt.txt', 'Delimiter', 'tab');

% 5.4 Verilog 半系数赋值
export_half_coeff_for_verilog('fir_coeff_half_for_verilog_wordlen_opt.txt', ...
                              coeff_half_int, ...
                              best_coeff_w, ...
                              best_frac_w, ...
                              best_n, ...
                              best_prune_thr, ...
                              '4x FIR word-length optimized half coefficients');

% 5.5 摘要文件
fid = fopen('interp4_wordlen_opt_summary.txt', 'w');
fprintf(fid, '4x FIR word-length optimization summary\n');
fprintf(fid, '=======================================\n');
...
fprintf(fid, '\nMode 176.4k:\n');
fprintf(fid, 'Ripple_pm_db = %.8f\n', best_res_1764.ripple_pm_db);
fprintf(fid, 'Stop_attn_db = %.8f\n', best_res_1764.stop_attn_db);
fprintf(fid, '\nMode 192k:\n');
fprintf(fid, 'Ripple_pm_db = %.8f\n', best_res_1920.ripple_pm_db);
fprintf(fid, 'Stop_attn_db = %.8f\n', best_res_1920.stop_attn_db);
fclose(fid);
...
fprintf('\n已导出文件：\n');
fprintf('1) interp4_wordlen_sparse_search_result.csv\n');
fprintf('2) interp4_wordlen_opt_summary.txt\n');
fprintf('3) fir_coeff_decimal_wordlen_opt.txt\n');
fprintf('4) fir_coeff_half_decimal_wordlen_opt.txt\n');
fprintf('5) fir_coeff_half_for_verilog_wordlen_opt.txt\n');
fprintf('6) interp4_wordlen_opt_response.png\n\n');
...
function plot_two_mode_response(b, Fs1, Fs2, f_pass_high, filename)
    ...
    ylabel('Magnitude (dB)');
    title('4x FIR Word-Length Optimized Frequency Response');
