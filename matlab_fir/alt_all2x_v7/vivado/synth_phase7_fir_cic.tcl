#=============================================================
# 文件名       : synth_phase7_fir_cic.tcl
# 脚本名       : synth_phase7_fir_cic
# 功能简述     : 对 Phase 7 FIR-CIC N=3/N=4 混合插值链执行
#                独立综合，导出总资源、层次资源、DSP 映射、
#                48MHz 时序、功耗估算和综合检查点。
#
# 当前默认配置：
#                  FPGA 型号：xc7a35tfgg484-2
#                  顶层模块：interp128_all2x_v7_fir_cic_top_ce
#                  前三级  ：Phase 6 24/22/20bit FIR
#                  尾链    ：15tap Q12 补偿 FIR + CIC16
#                  系统时钟：48MHz，周期 20.833ns
#                  参数    ：argv[0] = CIC_ORDER 3/4
#
# 设计作者     : kafeizizi
# 创建日期     : 2026-07-13
# 版本         : V2018.3
# 开发工具     : Vivado
# 修订记录     :
#                2026-07-13：新增 Phase 7 N=3/N=4 综合脚本。
#=============================================================
...
set v7_src_dir [file join $src_dir all2x_v7]

set cic_order 3
if {[llength $argv] > 0} {
    set cic_order [lindex $argv 0]
}
if {$cic_order == 3} {
    set final_prune_lsb 3
} elseif {$cic_order == 4} {
    set final_prune_lsb 6
} else {
    error "CIC order must be 3 or 4."
}

set result_dir [file normalize [file join $script_dir .. \
    vivado_results fir_cic_n${cic_order}]]
set temp_root [file normalize [file join $repo_dir \
    .codex_xvlog_check phase7 vivado_work]]
set temp_project_dir [file join $temp_root \
    phase7_fir_cic_n${cic_order}_[pid]]
...
set source_files [list \
    ...
    [file join $v7_src_dir cic_compensation_fir_ce.v] \
    [file join $v7_src_dir cic_interp16_core_ce.v] \
    [file join $v7_src_dir cic_interp16_top.v] \
    [file join $v7_src_dir interp128_all2x_v7_fir_cic_top_ce.v]]

set xdc_path [file join $result_dir phase7_clock_48m.xdc]
set xdc_fid [open $xdc_path w]
puts $xdc_fid {create_clock -name clk_audio_48m -period 20.833 [get_ports clk]}
close $xdc_fid

create_project phase7_fir_cic $temp_project_dir \
...
set_property include_dirs [list $v2_src_dir $v3_src_dir $v4_src_dir \
    $v5_src_dir $v6_src_dir $v7_src_dir] [get_filesets sources_1]
set_property top interp128_all2x_v7_fir_cic_top_ce \
    [get_filesets sources_1]
set_property generic [list STAGE23_ACC_W=38 CIC_ORDER=$cic_order \
    FINAL_PRUNE_LSB=$final_prune_lsb] [get_filesets sources_1]
update_compile_order -fileset sources_1

launch_runs synth_1 -jobs 4
wait_on_run synth_1
if {[get_property PROGRESS [get_runs synth_1]] != "100%"} {
    error "Phase 7 FIR-CIC synthesis did not complete."
}
...
puts "Phase 7 FIR-CIC N=$cic_order synthesis completed."
close_project
