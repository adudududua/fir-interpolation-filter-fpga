#=============================================================
# 文件名       : synth_phase7_folded_fir_cic.tcl
# 脚本名       : synth_phase7_folded_fir_cic
# 功能简述     : 对 Phase 7 折叠补偿 FIR-CIC N=3/N=4 插值链执行
#                独立综合，导出总资源、层次资源、DSP 映射、
#                48MHz 时序、功耗估算和综合检查点。
#
# 当前默认配置：
#                  FPGA 型号：xc7a35tfgg484-2
#                  顶层模块：interp128_all2x_v7_folded_fir_cic_top_ce
#                  前三级  ：24/22/20bit，Stage3 折叠 CIC 补偿
#                  尾链    ：CIC16
...
# 开发工具     : Vivado
# 修订记录     :
#                2026-07-13：新增折叠补偿 N=3/N=4 综合脚本。
#=============================================================
...
if {$cic_order == 3} {
    set final_prune_lsb 0
} elseif {$cic_order == 4} {
    set final_prune_lsb 7
} else {
    error "CIC order must be 3 or 4."
}

set result_dir [file normalize [file join $script_dir .. \
    vivado_results folded_fir_cic_n${cic_order}]]
...
set temp_project_dir [file join $temp_root \
    phase7_folded_fir_cic_n${cic_order}_[pid]]
...
create_project phase7_folded_fir_cic $temp_project_dir \
...
set_property top interp128_all2x_v7_folded_fir_cic_top_ce \
...
if {[get_property PROGRESS [get_runs synth_1]] != "100%"} {
    error "Phase 7 folded FIR-CIC synthesis did not complete."
}
...
puts "Phase 7 folded FIR-CIC N=$cic_order synthesis completed."
close_project
