# 📊 218-LUT 高阶数字插值滤波器补充测试

| 项目 | 内容 |
|---|---|
| 测试日期 | 2026-08-12 |
| 测试对象 | Vivado 2025.2 板测基线，Stage1/2/3=`24/20/20` |
| 配置标识 | `NF-P3-STAGE123-24-20-20-CANDIDATE-R1` |
| FPGA | XC7A35T-FGG484-2 |
| 验证状态 | 数字域补充实验通过；218-LUT 版本功能板测已确认 |

## 🧾 1. 补充测试执行概览

补充测试将证据划分为数值正确性、信号质量、RTL 位真、实现结果和物理测量五个层级，避免将
“RTL 0 LSB”等同于“无定点损失”，或将“示波器有波形”等同于“阻带达到 70 dB”。各实验
按照证据可获得性形成以下完成状态：
...
内部饱和。统一启动区与稳态区分离统计后，稳态输出触顶为 0，镜像抑制 88.232 dBc，故正式
...
E0～E3 的核心数字域测试已形成有效补充证据，E4 的资源链可用于说明优化演进。当前证据仍存在
以下边界：221/218 的 3-LUT 差异缺少三组配对 seed；动态功耗缺少至少两个 128× 工况的 SAIF；
频谱实验尚未覆盖双音、白噪声和 −60 dBFS；仪器级音频测试、OOC 多 seed Fmax 统计尚未完成。
...
## ⚙️ 10. 固定 2 DSP / 4 RAMB18E1 条件下的 LUT 优化补充试验
...
## ⚖️ 11. 218-LUT 与 239-LUT 板测版本的同口径核心 OOC 与整板资源对比
