...
vector_dir = fullfile(script_dir, 'vectors', 'route2c');
...
writetable(manifest, fullfile(result_dir, ...
    'route2c_bittrue_manifest.csv'));
disp(manifest);
fprintf('Route 2C bit-true vectors generated: %s\n', vector_dir);


function [name_out, input_n, y4_n, y8_n, y128_n, sat_n] = ...
        build_write_case(vector_dir, case_name, input_data)
    result = route2c_build_bittrue_case(input_data);
