...
> 状态更新（2026-08-12）：239 LUT / 388 FF / 3 DSP 配置随后已写入正式工程，完成
> bitstream 构建，并由用户确认物理板验证通过。本报告中的“未生成板测 bitstream”与
> “下一板测候选”仅描述固定资源压缩实验结束时的历史状态。
...
当前 **239 LUT / 388 FF / 3 DSP / 4 RAMB18E1** 已由下一板测候选升级为正式实板通过的
3-DSP Pareto 配置。
若还要显著压缩 LUT，优先级应为：

1. 评分公式明确后，可在两套均已板测的 239-LUT/3-DSP 与 218-LUT/4-DSP 配置之间选择；
...
固定资源压缩实验本身未修改正式 3-DSP RTL 和 `.xpr` 泛型，也未生成新 bitstream；
后续正式工程切换、bitstream 和板测证据独立归档于
`XC7A35T_interp_opt_2025.2/tools/vivado_2025_2/results/active_239lut_3dsp`。
