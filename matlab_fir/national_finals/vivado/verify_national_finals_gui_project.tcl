...
require_generic $project_generics USE_NATIONAL_FINALS_CIC_INTEGRATOR_DSP_MODE 1
...
if {[get_property PROGRESS [get_runs impl_1]] eq "100%"} {
    ...
    require_condition [expr {$dsp_count == 3}] \
        "GUI implementation is not the P4-C 3-DSP architecture."
    ...
    require_condition [expr {$lut_count <= 525}] \
        "GUI implementation exceeds the 525-LUT P4-C 3-DSP guard."
    require_condition [expr {$ff_count <= 515}] \
        "GUI implementation exceeds the 515-FF P4-C 3-DSP guard."
