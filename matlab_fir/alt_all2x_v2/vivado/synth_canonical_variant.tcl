#=============================================================
# 文件名       : synth_canonical_variant.tcl
# 脚本名       : synth_canonical_variant
# 功能简述     : 对 V2 canonical halfband7 指定尾级候选执行独立综合，
#                导出同口径资源利用率、时序报告和综合后检查点。
#
#                调用参数：
#                  argv[0]：FIRST_CANONICAL_STAGE，取 4、5、6、7
#
#                输出目录：
#                  ../vivado_results/stage<argv[0]>/
#
# 当前默认配置：
#                  FPGA 型号：xc7a35tfgg484-2
#                  顶层模块：interp128_all2x_v2_top_ce
#                  时钟频率：5.6448MHz
#
# 设计作者     : kafeizizi
# 创建日期     : 2026-07-11
...
# 开发工具     : Vivado
# 修订记录     :
#                2026-07-11：新增 V2 canonical 候选独立综合脚本。
#=============================================================

if {$argc != 1} {
    error "Usage: vivado -mode batch -source synth_canonical_variant.tcl -tclargs <4|5|6|7>"
}

set first_stage [lindex $argv 0]
if {$first_stage ni {4 5 6 7}} {
    error "FIRST_CANONICAL_STAGE must be 4, 5, 6 or 7."
}
...
set result_dir [file normalize [file join $script_dir .. \
    vivado_results stage${first_stage}]]
...
read_verilog [list \
    ...
    [file join $v2_src_dir interp128_all2x_v2_top_ce.v]]

synth_design -top interp128_all2x_v2_top_ce \
    -part xc7a35tfgg484-2 \
    -generic FIRST_CANONICAL_STAGE=$first_stage
...
puts "V2 canonical synthesis completed: FIRST_CANONICAL_STAGE=$first_stage"
