%% 1）主流程：v3_04_compare_stage1_rtl
% 功能说明：在相同输入下对齐 MATLAB 黄金模型与 RTL 输出，执行逐样本 bit-true 判定。

clc; clear;

%=============================================================
% 文件名       : v3_04_compare_stage1_rtl.m
% 脚本名       : v3_04_compare_stage1_rtl
% 功能简述     : 将 V3 strict Stage 1 完整七级 RTL 输出与 MATLAB
%                128x bit-true golden 进行固定延迟搜索和逐点对拍。
%
%                输出文件：
%                  stage1_strict_rtl_results.csv
...
% 开发工具     : MATLAB / Vivado
% 修订记录     :
%                2026-07-11：新增 V3 Stage 1 完整链路 RTL 对拍。
%=============================================================
...
rtl_name = {'v3_strict_impulse_output.csv'; ...
...
fprintf(fid, 'V3 Stage 1 strict-halfband RTL comparison\n');
fprintf(fid, '==========================================\n');
...
disp(result_table);
if ~all(row_pass)
    error('V3 Stage 1 完整链路 RTL 对拍未达到 0 LSB。');
end
fprintf('V3 Stage 1 冲激与随机 PCM 完整链路对拍均为 0 LSB。\n');
