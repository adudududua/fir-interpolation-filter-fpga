...
## 当前最低 LUT 工具签核候选：Vivado 2025.2 249 LUT / 4 DSP（2026-08-08）
...
DAC 数据均持续变化且无 X。当前状态为 **tool-verified，待用户物理板复测**；在复测确认前，
258-LUT 版仍是正式实板安全回退，本文不把仿真结果写成板测结果。
工具签核标签为 `nf-vivado2025.2-249lut-377ff-4dsp-2bram-toolverified`。
...
| **2025.2 CIC DSP角色交换工具候选** | **全国赛板级** | **249** | **377** | **4** | **2** | **2** | **+43.997/+0.085 ns** | **0.271 W** | **三级comb串行减法进入DSP PREG，第一级26-bit积分器换至CARRY4，并复用comb索引作对齐状态；较258少9 LUT/多1 FF** | **CIC 4096输出定向等价、Smoke/Release 17/17、三节点0-LSB、实现/DRC/bitstream及routed六档通过；待用户物理板复测** |
...
- **当前最低 LUT 工具签核候选为 Vivado 2025.2 CIC DSP 角色交换版：249 LUT / 377 FF / 4 DSP / 2 BRAM Tile**；已完成 CIC 4096 输出定向等价、Smoke/Release 17/17、14组全链三节点0-LSB、正式实现/DRC/时序/功耗/bitstream及 routed 六档门禁，当前待用户物理板复测；
- **当前最低 LUT 且正式实板通过版为 Vivado 2025.2 Stage1顺序抽头版：258 LUT / 376 FF / 4 DSP / 2 BRAM Tile**；相对276-LUT基线少18 LUT/3 FF，Release 17/17、全链三节点0-LSB、正式实现/DRC/bitstream、routed六档以及用户物理板各档采样率/DAC波形均通过；
- **前一 Vivado 2025.2 实板安全回退为276 LUT / 379 FF / 4 DSP / 2 BRAM Tile**；292 LUT版本继续作为更早实板回退；
...
## 全国总决赛当前最低 LUT 工具签核候选（2026-08-07，P3-U 333-LUT 控制路径优化版）

当前分支为 `national-finals-p3u-4dsp-deep-optimization`。P3-U 保留 P3-T 的全部滤波系数、定点定义、Stage1/Stage2/3/CIC 数据调度和板级接口，只把正式参数的键盘消抖改为精确 Johnson 序列，并把模式 CDC 的 pending flag 加倒计数器改为周期完全一致的 one-hot settle token。综合为 **365 LUT / 384 FF**，完整板级实现为 **333 LUT / 382 FF / 152 Slice / 4 DSP / 2 BRAM Tile / 2 MMCM**；相对 P3-T 减少 8 LUT 和 11 Slice，仅增加 1 FF。WNS/WHS=`+45.704/+0.079 ns`，AD9708 setup/hold=`+76.116/+78.117 ns`，vectorless 总/动态/静态功耗=`0.271/0.199/0.072 W`。

工具侧验证采用完整口径：Smoke/Release 均为 17/17；Release 覆盖冲激、10 seed、正负满量程、强 −1 dBFS、4x/8x/128x 所有节点0 LSB、8类复位、1200次CDC、100次时钟切族和10次动态切档。正式 routed DCP 的默认 DAC 测试得到11290个边沿/7462次数据变化，六档得到44.1 kHz的`177/353/5645 edges/ms`与48 kHz的`192/384/6144 edges/ms`，六档DAC数据均变化。普通GUI工程从`reset_run synth_1`开始重建并成功Generate Bitstream，再次得到333 LUT/382 FF/152 Slice/4 DSP/4 RAMB18；手动工程策略已统一为`AreaOptimized_high/full/on + ExploreWithRemap + Explore`。正式结果见 [`p3u_333lut_382ff_4dsp_2bram_signedoff`](matlab_fir/national_finals/vivado_results/p3u_333lut_382ff_4dsp_2bram_signedoff) 与 [P3-U执行反馈](matlab_fir/national_finals/results/p3u_333lut_control_optimization_execution_feedback.md)。**P3-U 尚待用户物理板测试，不能标为 boardverified。**

## 全国总决赛当前正式实板通过安全回退（2026-08-06，P3-T 341-LUT Stage1 DSP尾周期版）

P3-T 让 Stage1 已有 DSP48E1 在 MAC 后的空闲周期完成精确 Q15 舍入、Pattern 符号扩展检查和 PREG 直接饱和钳位，并由稳定写指针派生历史基址。完整板级资源为 **341 LUT / 381 FF / 163 Slice / 4 DSP / 2 BRAM Tile / 2 MMCM**，WNS/WHS=`+45.270/+0.107 ns`，功耗为0.271 W。Smoke/Release 17/17、全链0-LSB、GUI重建、routed-DCP默认DAC/六档与bitstream均通过；用户实板确认44.1/48 kHz两族4x/8x/128x采样率正确且DAC波形正常。安全标签为`nf-p3t-final-341lut-381ff-163slice-4dsp-2bram-boardverified`。

## 全国总决赛前一实板通过回退（2026-08-05，P3-M 368-LUT Stage1 DSP-register 版）

P3-M 在 P3-L 的单 RAMB18 Stage1 上重新安排52次读操作：当前输入不再绕过 RAM 进入24-bit Slice寄存器，而是写入后读回；每个左样本由DSP48E1 `AREG`保存到随后的右样本MAC，`INMODE[1:2]`直接在预加器内部屏蔽无效历史。该方法保持1颗Stage1 DSP、1颗Stage2/3 DSP、2颗CIC DSP和2个BRAM Tile不变。

独立单进程与普通GUI工程从零重建均得到 **368 LUT / 386 FF / 156 Slice / 4 DSP / 2 BRAM Tile / 2 MMCM**。最终同源码 Smoke/Release 均为 **17/17 PASS**；Release的14组正式输入在4x/8x/128x全部0 LSB，真实RAMB18原语Stage1比较1400输出0 LSB；WNS/WHS=`+44.836/+0.117 ns`，功耗`0.271 W`，默认与六模式routed-DCP DAC活动和采样率计数全部通过。**2026-08-05 用户实板确认各档采样率正确、DAC输出波形正常，P3-M正式升级为当前发布。**完整记录见 [P3-M 执行反馈](matlab_fir/national_finals/results/p3m_stage1_dspreg_368lut_execution_feedback.md)。

## 全国总决赛更早实板通过回退（2026-08-04，P3-L 397-LUT 共享保护时基版）
...
> **版本口径说明**：当前最低 LUT 工具签核候选是 Vivado 2025.2 的 249 LUT / 377 FF / 4 DSP / 2 BRAM / 2 MMCM CIC DSP角色交换版，完整RTL、实现与routed六档均通过，待物理板复测；当前最低 LUT 且正式实板通过版仍是258 LUT / 376 FF / 4 DSP / 2 BRAM / 2 MMCM Stage1顺序抽头版。前一2025.2实板回退是276 LUT / 379 FF，292 LUT版本为更早回退。P3-U 333 LUT、P3-T 341 LUT、P3-S 343 LUT、P3-R 348 LUT、P3-M 368 LUT与P3-L 397 LUT为更早回退。旧 412-LUT P3-K 与424-LUT P3-J虽然真Q15和RTL 0-LSB均通过，但测试音ROM在综合后锁死，不能上板；历史424 LUT / 6 DSP Route 1和436 LUT / 5 DSP版另有Stage3约−6.02 dB标度错误。

> 当前最低 LUT 工具签核候选：Vivado 2025.2 CIC DSP角色交换版，249 LUT / 377 FF / 4 DSP / 2 BRAM Tile / 2 MMCM（待板测）<br>
> 当前最低 LUT 正式实板版：Vivado 2025.2 Stage1顺序抽头版，258 LUT / 376 FF / 4 DSP / 2 BRAM Tile / 2 MMCM<br>
> 前一 Vivado 2025.2 实板回退：276 LUT / 379 FF / 4 DSP / 2 BRAM Tile / 2 MMCM<br>
