...
本节按时间顺序统一整理“最初 4x+2x 结构、区域赛全 2x 优化、FIR-CIC 优化、全国赛全 2x 回退对照、全国赛 6-DSP CIC 以及当前 440-LUT 版本”。后文保留各阶段的设计、仿真、实现和板测原始记录，作为本节结论的详细证据。
...
最初 4x + 5×2x
  ...
  -> 全国赛 6-DSP CIC：573 LUT 基线连续优化到当前 440 LUT
```
...
### 第五阶段：全国赛 6-DSP CIC 从 573 LUT 优化到 440 LUT
...
| 全国赛 6-DSP CIC 版本 | LUT | LUT Δ | FF | FF Δ | Slice | DSP | BRAM | MMCM | 功耗估计 | 主要方法 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 双采样率功能基线 | 602 | — | 616 | — | — | 8 | 3 | 2 | 0.271 W | 44.1/48 kHz、三正式节点 |
| 6-DSP 回退基线 | 573 | -29 | 621 | +5 | 255 | 6 | 3 | 2 | 0.271 W | comb 三周期串行化，8 DSP 降到 6 DSP |
| 实现策略改为 `Default` | 556 | -17 | 621 | 0 | 229 | 6 | 3 | 2 | — | 同一综合 DCP 小范围策略实扫 |
| DSP PREG 状态复用 | 556 | 0 | 557 | -64 | 231 | 6 | 3 | 2 | — | CIC 内部状态留在原生 PREG |
| 组合均衡直交 CIC | 557 | +1 | 536 | -21 | 228 | 6 | 3 | 2 | — | 删除冗余寄存边界 |
| comb 历史轮转/字长压缩 | 528 | -29 | 532 | -4 | 229 | 6 | 3 | 2 | — | 22-bit 轮转历史、Stage1 41-bit |
| Stage2/3 累加器复用器消除 | 487 | -41 | 532 | 0 | 205 | 6 | 3 | 2 | — | 首抽头直接使用已清零 PREG |
| FIR PREG 累加与提交 | 461 | -26 | 464 | -68 | 199 | 6 | 3 | 2 | — | Stage1、Stage2/3 删除外部宽累加寄存器 |
| DSP `CARRYIN` 精确舍入 | 451 | -10 | 464 | 0 | — | 6 | 3 | 2 | — | 空闲提交周期实现正负对称 Q15 舍入 |
| **当前正式版** | **440** | **-11** | **464** | **0** | **191** | **6** | **3** | **2** | **0.271 W** | Stage3 最坏界证明后删除不可能触发的饱和逻辑 |

573-LUT 基线到当前版本的总变化为：
...
| 版本 | 口径 | LUT | FF | DSP | BRAM | MMCM | 功耗估计 | 相对关键基线的变化 | RTL/实现/物理板 |
|---|---|---:|---:|---:|---:|---:|---:|---|---|
| 最初 `4x+5×2x` | 独立链 | 9002 | 4497 | 2 | 0 | — | — | 最初功能基线 | MATLAB/RTL；早期资料未统一板级口径 |
| 全 2x 首个稳定版 | 独立链 | 5912 | 4218 | 1 | 0 | — | — | 比最初独立链少 3090 LUT、279 FF、1 DSP | RTL/综合通过 |
| Phase 6 全 2x | 单采样率板级 | 1395 | 1040 | 2 | 1 | 1 | 0.168 W | 比全 2x 初始板级少 5031 LUT、3376 FF | RTL/实现/bitstream/物理板均通过 |
| FIR-CIC 首版 | 单采样率板级 | 1128 | 964 | 2 | 1 | 1 | — | 比 Phase 6 少 267 LUT、76 FF | RTL/实现通过 |
| FIR-CIC 低 DSP | 单采样率板级 | 941 | 940 | 2 | 1.5 | 1 | 0.168 W | 用 0.5 BRAM 再减 187 LUT、24 FF | RTL/实现/bitstream |
| FIR-CIC 低 LUT 中间版 | 单采样率板级 | 578 | 619 | 9 | 1.5 | 1 | 0.168 W | 比 941-LUT 版少 363 LUT、321 FF，增加 7 DSP | 完整 RTL/实现/bitstream |
| 区域赛 FIR-CIC 最终版 | 单采样率板级 | 472 | 564 | 8 | 3 | 1 | 0.169 W | 比 578-LUT 版少 106 LUT、55 FF、1 DSP，增加 1.5 BRAM | **RTL/实现/bitstream/物理板均通过** |
| 全国赛功能基线 | 全国赛板级 | 602 | 616 | 8 | 3 | 2 | 0.271 W | 新增双采样率与三正式节点，范围发生变化 | RTL/实现/bitstream |
| 全国赛全 2x，最低 LUT | 全国赛板级 | 714 | 662 | 3 | 3 | 2 | 0.271 W | 比 573-LUT CIC 多 141 LUT、41 FF，少 3 DSP | RTL/实现/bitstream；未物理板测 |
| 全国赛全 2x，最低 DSP | 全国赛板级 | 728 | 662 | 2 | 3 | 2 | 0.271 W | 比 573-LUT CIC 多 155 LUT、41 FF，少 4 DSP | RTL/实现/bitstream；未物理板测 |
| 全国赛 6-DSP CIC 基线 | 全国赛板级 | 573 | 621 | 6 | 3 | 2 | 0.271 W | 比 602-LUT/8-DSP 基线少 29 LUT、2 DSP | 9/9 RTL、实现、bitstream；未物理板测 |
| **当前全国赛正式版** | **全国赛板级** | **440** | **464** | **6** | **3** | **2** | **0.271 W** | **比 573 基线少 133 LUT、157 FF，DSP/BRAM/MMCM 不变** | **9/9 RTL、GUI/脚本实现、bitstream；待物理板测** |
...
- 440-LUT 正式版已经完成 MATLAB、RTL、XSim、综合、布局布线、时序、DRC、GUI 复现和 bitstream 签核，最后尚缺的是当前双采样率 bitstream 的物理板下载与仪器验收，不能把区域赛 472-LUT 版本的实板结论直接代替它。

## 全国总决赛双采样率版本（2026-07-31，6-DSP CIC 第五轮优化详细签核）

全国赛升级版当前位于分支 `codex/national-finals-cic6-gui-fix-round5-opt`：支持 signed 24 bit、44.1/48 kHz 输入家族以及 4x/8x/128x 正式输出。最终 RTL 六工况的通带最大绝对偏差为 **0.004610～0.006918 dB**，阻带衰减为 **72.348～78.669 dB**，冲激对称误差均为 **0 LSB**；完整 XSim 回归 **9/9 PASS**，全链路 impulse + 随机 PCM 的 4x/8x/128x 三节点均为 **0 LSB mismatch**，另通过 8 个内部状态复位场景和 10 次不停机倍率切换。
