...
### 2026-08-12：218-LUT与239-LUT板测检查点的模块级资源审计
...
路由后检查点执行只读资源审计；主工程保持 268-LUT/2-DSP 分支不变。正式工程采用全层次扁平化，
故普通层次利用率报告只能给出顶层总量。本次统计以物理 LUT BEL、Slice Register、DSP48E1 和
RAMB18E1 为单位，依据 RTL 源位置及寄存器/DSP/BRAM 连通端点恢复模块归属，并将跨模块 LUT
单独列为共享逻辑。两版汇总分别严格回归 **218/365/4/4** 与 **239/388/3/4**。

主要子模块对比如下：

| 模块 | 218版 LUT/FF | 239版 LUT/FF | DSP 218/239 | RAMB18E1 218/239 |
|---|---:|---:|---:|---:|
...
差分集中在 16 倍 CIC。239 版采用 `CIC_INTEGRATOR_DSP_MODE=1`，218 版采用模式 2；
新增 `u_cic_comb_dsp48e1` 后，CIC 从 57 LUT/132 FF/1 DSP 变为 34 LUT/109 FF/2 DSP，
即以 1 个 DSP 换回 **23 LUT 与23个 Slice Register**。由239版切换至218版时，FIR1级、
2→4倍桥接量化和核心—外围接口共享逻辑的物理LUT分别变化+1、−1和+2，共产生
+2 LUT偏移，形成整板
**−21 LUT/−23 FF/+1 DSP**；4个 RAMB18E1保持不变。两版WNS分别为+45.279 ns与+44.703 ns，
WHS均为+0.079 ns，DRC Error均为0。跨层次共享LUT数据仅适用于这两个已签核检查点，不能作为
独立模块的通用面积常数。技术报告已补入表9-16、图9-7及相应分析；统计明细位于
[模块资源对比CSV](submit/finally/images/work/resource_compare_218_239/analysis/module_resource_comparison.csv)。
