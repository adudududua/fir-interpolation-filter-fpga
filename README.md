...
六组创新验证实验及图表见
[创新点验证实验报告](submit/finally/reports/创新点验证实验报告.md)。消融方法已按严格口径修正：
主结构消融不使用已包含字长、时分复用和资源共享的近期相邻版本作零优化基线，而是回到
全并行、24-bit级间数据、无时分复用和无共享的真实原始点
**9376 LUT / 4118 FF / 38 DSP / 0 BRAM Tile**。同一Vivado 2018.3口径逐项加入
Stage1时分复用、后级资源映射、canonical halfband、Stage2/3 polyphase共享、
BRAM历史、共享DSP、紧凑舍入和混合字长后，独立全2×链达到
**1224 LUT / 867 FF / 2 DSP / 1 BRAM Tile**。近期221/218/239/268-LUT版本只用于
现代字长局部Pareto、DSP/功耗和Fmax对照，不再冒充无优化基线。

同工具Vivado 2025.2 OOC架构实验进一步得到：正式FIR–CIC 3-DSP核心为
**212 LUT / 302 FF / 3 DSP / 3 RAMB18E1**，全2×混合字长核心为
**1045 LUT / 826 FF / 2 DSP / 2 RAMB18E1**；前者减少833 LUT和524 FF，代价为
增加1 DSP和1 RAMB18E1。联合Stage3–CIC补偿、24/20/20字长、4/3/2-DSP SAIF功耗、
83.33/100/125 MHz核心扫描和17/17 RTL/0-LSB/路由后六模式证据均已在报告中分别列出，
物理板结论仍只赋予用户已确认的239-LUT正式版本。

新增CIC严格等效Hold消融：保持前三级FIR、联合补偿、24/20/20字长、器件、时钟和
实现策略不变，无Hold旧N3核心为 **188 LUT / 280 FF / 5 DSP / 3 RAMB18E1**，
`C²→Hold16→I²`核心为 **190 LUT / 279 FF / 4 DSP / 3 RAMB18E1**，两者时序和DRC均通过。
Release同时比较旧N3和Hold的4/3/2-DSP映射，在连续、随机停顿、10组随机种子和burst复位下
共53,760个完成输出样点data/valid逐拍0 LSB。因此这是以 **+2 LUT、−1 FF换−1 DSP** 的
DSP优先Pareto创新。CIC/Noble/Hold恒等式本身属于已有多速率理论；本项目的可主张创新是其
双采样率多档音频RTL化、26/29-bit状态证明、专用资源映射和完整验证闭环，而不是新数学公式。

### 2026-08-12：218-LUT与239-LUT板测版本的同口径核心OOC与整板资源复核

已在隔离 Git 工作树中依次检出
`nf-vivado2025.2-218lut-365ff-4dsp-2bram-24-20-20-board-pass` 与
`nf-vivado2025.2-239lut-388ff-3dsp-2bram-24-20-20-board-pass`，对两个 Vivado 2025.2
路由后检查点执行只读复核；主工程保持 268-LUT/2-DSP 分支不变。本次将统计边界严格分为两类：
完整系统资源取自对应板级 routed checkpoint；插值滤波器核心资源则从两个 board-pass
标签分别导出不可变 RTL 快照，在相同 Vivado 2025.2、器件、OOC 顶层、时钟约束和实现指令下
重新进行 post-route，仅使 `CIC_INTEGRATOR_DSP_MODE=2/1` 与两个板测版本精确匹配。
...
218版核心内部寄存器路径WNS/WHS为`+150.943/+0.069 ns`，239版为
`+151.611/+0.099 ns`；两者DRC Error均为0，且均从第二个空结果目录精确复现（2/2一致）。
零I/O延迟的独立边界模型在两版中各产生1条相同的端口到寄存器hold违例（−0.845 ns），
因此上述裕量不表述为OOC整体时序通过；完整接口时序以对应板级post-route签核为准。OOC核心包含三级2倍FIR、
级间桥接/量化、16倍CIC及核心内部控制和3个RAMB18E1，不包含测试音频ROM、按键/模式控制、
时钟管理、DAC/IO和板级包装。整板扁平化网表的物理单元归属仅用于辅助定位差异集中于CIC映射，
不作为模块独立面积，也不与OOC结果相减推导外围资源。技术报告已更新表9-16、图9-7及结论；
正式证据位于[匹配OOC重复实现记录](submit/finally/images/work/resource_compare_218_239/matched_ooc/evidence/matched_ooc_repeated_runs.csv)。

当前268-LUT版本对应提交 `47b2921`，并已打上工具签核标签
`nf-vivado2025.2-268lut-417ff-2dsp-2bram-24-20-20-toolverified`；该标签不表述为物理板验证。

### 2026-08-12：当前工程固定为 268 LUT / 2 DSP，并完成固定资源优化复验
...
Stage1/Stage2/Stage3=`24/20/20 bit`。RTL Smoke 与 Release 均为 **17/17 PASS**；
Vivado 2025.2 完整综合、实现和 bitstream 均通过，WNS/WHS=`+44.389/+0.078 ns`，
DRC Error=0。最新完整构建归档为
`XC7A35T_interp_opt_2025.2/tools/vivado_2025_2/results/20260812_205122`。
...
**tool-verified**；工具结果与物理板结论分层记录。

在保持 2 DSP、4 RAMB18E1、2 MMCM、24/20/20 bit、完整输入域和原吞吐不变的条件下，
本轮继续扫描综合/实现策略，并对 pending/burst 状态合并、舍入状态表示和共享 Fabric
加法器进行位精确与完整板级实现复验。三项结构候选分别为 271、288 和 295 LUT，均未低于
268-LUT 基线；共享加法器还不满足 128 倍突发期每拍输出的吞吐约束。当前正式 RTL 已恢复为
268-LUT 基线，未采纳候选未写入生产数据通路。该结论限定于本轮已覆盖的结构与策略空间，
不表述为数学意义上的全局最优。详见
...
`XC7A35T_interp_LUTmin218_DSP4_2025.2`，作为 LUT 优先的独立回退。
...
| 0 | 329 / 425 | **268 / 417** | **2** | 4 | 2 | +44.389/+0.078 ns | Smoke/Release 17/17 | 工具验证 Pareto |
...
`29×LUT权重 + 29×FF权重`。当前正式工程使用已经 Vivado 2025.2 完整验证的
268/2-DSP 版；已板测的 239/3-DSP 与 218/4-DSP 版分别保留为均衡回退点和 LUT 优先回退点。
三点无 SAIF 功耗估算均为
...
| **历史239-LUT/377-FF/4-DSP Stage1延迟使能版核心OOC（mode2）** | **核心独立实现** | **197** | **290** | **4** | **1.5** | **0** | **内部 +151.665/+0.054 ns** | **—** | **仅对应历史239-LUT/377-FF/4-DSP版；3个RAMB18E1，不含板级ROM/控制/时钟/DAC/IO** | **post-route、0路由错误、OOC范围0 DRC Error；不用于当前239-LUT/388-FF/3-DSP版** |
...
| **当前239-LUT/388-FF/3-DSP版匹配核心OOC（mode1）** | **核心独立实现** | **212** | **302** | **3** | **1.5** | **0** | **内部 +151.611/+0.099 ns** | **—** | **与当前3-DSP板测标签的24/20/20参数精确匹配；3个RAMB18E1** | **两个空结果目录均精确复现，post-route、正内部时序、OOC范围0 DRC Error** |
...
- **当前生产工程为 Vivado 2025.2 CIC 模式0：268 LUT / 417 FF / 2 DSP / 2 BRAM Tile**；Smoke/Release 均为17/17，14组全链在4x/8x/128x三节点逐样本0 LSB，完整综合、布局布线、时序、DRC和bitstream均通过；该点保持tool-verified，物理板状态不由工具结果代替；
...
- **239 LUT / 388 FF / 3 DSP / 2 BRAM Tile 为低 DSP 的已板测 Pareto 回退版**；相对218-LUT版以21 LUT和23 FF换取1 DSP，Smoke 17/17、正式实现、正时序、0 DRC Error、bitstream及用户物理板均通过，并保留独立board-pass标签；
