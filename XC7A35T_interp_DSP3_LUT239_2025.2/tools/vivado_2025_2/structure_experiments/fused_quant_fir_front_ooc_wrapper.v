...
module fused_quant_fir_front_ooc_wrapper (
    input  wire                    clk,
    input  wire                    rst_n,
    input  wire                    ce2_out,
    input  wire                    ce4_out,
    ...
    wire y2_raw_valid;
    wire signed [23:0] y2_to_stage2_raw;
    wire y2_to_stage2_valid;
    wire signed [21:0] y4_raw;
    wire y4_raw_valid;
    wire signed [21:0] y4_to_stage3_raw;
    ...
    bridge_valid_raw_experiment_ce #(
        .DATA_W(24)
    ) u_raw_bridge_2_to_4 (
        .clk(clk), .rst_n(rst_n),
        .in_data(y2_raw), .in_valid(y2_raw_valid),
        .ce_out_next(ce4_out), .phase_current(stage2_phase),
        .out_data(y2_to_stage2_raw), .out_valid(y2_to_stage2_valid)
    );
    ...
    bridge_valid_raw_experiment_ce #(
        ...
        .out_data(y4_to_stage3_raw), .out_valid(y4_to_stage3_valid)
    );

    interp2_stage23_fused_quant_dsp_ce #(
        .DATA_W(24),
        .STAGE2_INPUT_W(24), .STAGE3_INPUT_W(22),
        .STAGE2_DATA_W(22), .STAGE3_DATA_W(20),
