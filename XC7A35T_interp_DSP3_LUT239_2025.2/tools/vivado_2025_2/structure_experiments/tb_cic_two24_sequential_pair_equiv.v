...
module tb_cic_two24_sequential_pair_equiv;
    reg clk = 1'b0;
    reg rst_n = 1'b0;
    reg ce_out = 1'b0;
    ...
    cic_interp16_n3_hold2_two24_sequential_pair_ce u_cand (
        .clk(clk), .rst_n(rst_n), .ce_out(ce_out),
        .x_in(x_in), .x_in_valid(x_in_valid),
    ...
    initial begin
        ...
        if (!reset_completed || mismatch_count != 0 || fifo_count != 0)
            $fatal(1,
                "SEQUENTIAL TWO24 EQUIV FAIL outputs=%0d mismatch=%0d fifo=%0d reset=%0b",
                outputs_checked, mismatch_count, fifo_count, reset_completed);
        $display("SEQUENTIAL TWO24 EQUIV PASS: outputs=%0d, fixed/random CE and mid-burst reset, 0 LSB",
                 outputs_checked);
        $finish;
    end
    ...
    initial begin
        #6000000;
        $fatal(1, "SEQUENTIAL TWO24 EQUIV TIMEOUT outputs=%0d fifo=%0d",
