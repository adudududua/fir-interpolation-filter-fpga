...
module tb_cic_shared_carry_mode1_equiv;
    ...
    cic_interp16_n3_hold2_shared_carry_mode1_candidate #(
        .DATA_W(DATA_W), .OUTPUT_W(OUTPUT_W),
        .FINAL_PRUNE_LSB(0), .BURST_COUNTER_USE_DSP(0)
    ) candidate_cic (
        .clk(clk), .rst_n(rst_n), .ce_out(ce_out),
    ...
    initial begin
        ...
        $display("SHARED-CARRY CIC EQUIVALENCE PASS: continuous=320 stalled=480 seeds=20x256 reset-mid-burst=PASS outputs=%0d 0 LSB",
            candidate_count);
        $finish;
    end
endmodule
