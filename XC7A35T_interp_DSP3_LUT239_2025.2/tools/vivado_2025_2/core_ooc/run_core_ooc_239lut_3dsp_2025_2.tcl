#=============================================================
# 文件名       : run_core_ooc_239lut_3dsp_2025_2.tcl
# 功能简述     : 以正式 24/20/20 位、3-DSP 参数独立实现滤波器核心，
...
# 正式 239-LUT/3-DSP 板级版本对应的独立核心参数，不能使用模块默认值。
...
puts $summary_fid "VARIANT=239lut_3dsp_wordlength_24_20_20"
...
# 门禁保证生成的不是默认 8-DSP 版本，也不是正式 4-DSP 版本。
require_condition [expr {$lut_count == 212}] "预期 212 LUT，实际 $lut_count。"
require_condition [expr {$lutram_count == 0}] "预期 0 LUTRAM，实际 $lutram_count。"
require_condition [expr {$ff_count == 302}] "预期 302 FF，实际 $ff_count。"
require_condition [expr {$dsp_count == 3}] "预期 3 DSP48E1，实际 $dsp_count。"
...
puts "CORE_OOC_239LUT_3DSP_2025_2_PASS"
close_design
exit
