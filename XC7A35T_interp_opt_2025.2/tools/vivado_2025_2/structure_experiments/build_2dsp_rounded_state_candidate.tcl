...
set candidate_core [file join $script_dir \
    cic_interp16_n3_hold2_rounded_state_ce.v]
set candidate_adapter [file join $script_dir \
    cic_rounded_state_dropin_adapter.v]
set result_dir [expr {$argc > 0 ? [file normalize [lindex $argv 0]] : \
    [file join $script_dir results two_dsp_rounded_state]}]
set jobs [expr {$argc > 1 ? [lindex $argv 1] : 1}]
set candidate_kind [expr {$argc > 2 ? [lindex $argv 2] : "rounded"}]

if {$candidate_kind eq "merged"} {
    set candidate_adapter [file join $script_dir \
        cic_merged_burst_dropin_adapter.v]
} elseif {$candidate_kind ne "rounded"} {
    error "Unknown candidate kind: $candidate_kind"
}
...
if {$candidate_kind eq "merged"} {
    set candidate_core [file join $result_dir \
        cic_hold2_merged_generated.v]
    set source_input [open $original_cic r]
    set source_text [read $source_input]
    close $source_input
    set source_output [open $candidate_core w]
    puts $source_output \
        "`define CIC_HOLD2_MODULE_NAME cic_hold2_merged_core"
    puts -nonewline $source_output $source_text
    puts $source_output "\n`undef CIC_HOLD2_MODULE_NAME"
    close $source_output
    set sandbox_dir [file join $project_dir .mb2dsp_sandbox]
} else {
    set sandbox_dir [file join $project_dir .rs2dsp_sandbox]
}
require_condition [expr {![file exists $sandbox_dir]}] \
    "Candidate sandbox already exists: $sandbox_dir"
...
foreach candidate_file [list $candidate_core $candidate_adapter] {
...
puts $manifest "BASE=268-LUT 24/20/20 2-DSP board-pass"
puts $manifest "CANDIDATE_KIND=$candidate_kind"
if {$candidate_kind eq "merged"} {
    puts $manifest "CANDIDATE=merged pending/countdown five-bit sentinel state"
} else {
    puts $manifest "CANDIDATE=exact quotient/residual final-integrator state"
}
puts $manifest "RTL_EQUIV=15984 outputs, cycle-exact, 0 LSB"
...
puts "TWO_DSP_ROUNDED_STATE_LUT=$lut_count"
puts "TWO_DSP_ROUNDED_STATE_FF=$ff_count"
puts "TWO_DSP_ROUNDED_STATE_DSP=$dsp_count"
puts "TWO_DSP_ROUNDED_STATE_BRAM18=$bram18_count"
puts "TWO_DSP_ROUNDED_STATE_WNS=$wns"
puts "TWO_DSP_ROUNDED_STATE_WHS=$whs"
puts "TWO_DSP_ROUNDED_STATE_DRC_ERRORS=[llength $drc_errors]"
...
puts "TWO_DSP_ROUNDED_STATE_CANDIDATE_PASS"
close_design
close_project
exit
