...
set candidate_core [file join $script_dir \
    cic_interp16_n3_hold2_shared_fabric_ce.v]
set candidate_adapter [file join $script_dir \
    cic_shared_fabric_dropin_adapter.v]
set result_dir [expr {$argc > 0 ? [file normalize [lindex $argv 0]] : \
    [file join $script_dir results two_dsp_shared_fabric]}]
...
foreach required_file [list $project_file $original_cic $packed_rom \
        $candidate_core $candidate_adapter] {
    require_condition [file isfile $required_file] \
        "Missing required file: $required_file"
}

file mkdir $result_dir
set sandbox_dir [file join $project_dir .sf2dsp_sandbox]
...
puts $manifest "CANDIDATE=one 29-bit Fabric adder shared by both CIC integrators"
...
puts "TWO_DSP_SHARED_FABRIC_LUT=$lut_count"
puts "TWO_DSP_SHARED_FABRIC_FF=$ff_count"
puts "TWO_DSP_SHARED_FABRIC_DSP=$dsp_count"
puts "TWO_DSP_SHARED_FABRIC_BRAM18=$bram18_count"
puts "TWO_DSP_SHARED_FABRIC_WNS=$wns"
puts "TWO_DSP_SHARED_FABRIC_WHS=$whs"
puts "TWO_DSP_SHARED_FABRIC_DRC_ERRORS=[llength $drc_errors]"
...
puts "TWO_DSP_SHARED_FABRIC_CANDIDATE_PASS"
close_design
close_project
exit
