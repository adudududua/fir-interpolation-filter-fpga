`timescale 1ns / 1ps

module tb_cic_rounded_state_equiv;
    reg clk = 1'b0;
    reg rst_n = 1'b0;
    reg ce_out = 1'b0;
    reg signed [20:0] x_in = 21'sd0;
    reg x_in_valid = 1'b0;

    wire signed [19:0] y_ref;
    wire y_ref_valid;
    wire signed [19:0] y_cand;
    wire y_cand_valid;

    integer seed = 32'h6d3185c7;
    integer inputs_sent = 0;
    integer outputs_since_input = 16;
    integer outputs_checked = 0;
    integer mismatch_count = 0;
    integer ce_gap = 0;
    reg reset_completed = 1'b0;

    always #5 clk = ~clk;

    cic_interp16_n3_hold2_dsp_ce #(
        .DATA_W(21), .OUTPUT_W(20), .FINAL_PRUNE_LSB(0),
        .BURST_COUNTER_USE_DSP(0), .INTEGRATOR_DSP_MODE(0)
    ) u_ref (
        .clk(clk), .rst_n(rst_n), .ce_out(ce_out),
        .x_in(x_in), .x_in_valid(x_in_valid),
        .y_out(y_ref), .y_out_valid(y_ref_valid),
        .burst_remaining_dbg(), .pending_dbg(), .comb_busy_dbg()
    );

    cic_interp16_n3_hold2_rounded_state_ce u_cand (
        .clk(clk), .rst_n(rst_n), .ce_out(ce_out),
        .x_in(x_in), .x_in_valid(x_in_valid),
        .y_out(y_cand), .y_out_valid(y_cand_valid),
        .burst_remaining_dbg(), .pending_dbg(), .comb_busy_dbg()
    );

    always @(negedge clk) begin
        if (!rst_n) begin
            ce_out <= 1'b0;
            x_in_valid <= 1'b0;
            outputs_since_input <= 16;
            ce_gap <= 0;
        end
        else begin
            if (y_ref_valid !== y_cand_valid) begin
                $display("VALID mismatch output=%0d ref=%0b cand=%0b",
                         outputs_checked, y_ref_valid, y_cand_valid);
                mismatch_count <= mismatch_count + 1;
            end
            if (y_ref_valid && y_cand_valid) begin
                if ($signed(y_ref) !== $signed(y_cand)) begin
                    $display("DATA mismatch output=%0d ref=%0d cand=%0d",
                             outputs_checked, $signed(y_ref), $signed(y_cand));
                    mismatch_count <= mismatch_count + 1;
                end
                outputs_checked <= outputs_checked + 1;
                outputs_since_input <= outputs_since_input + 1;
            end

            x_in_valid <= 1'b0;
            if (outputs_since_input >= 16 && inputs_sent < 1000) begin
                case (inputs_sent)
                    0: x_in <= 21'sd0;
                    1: x_in <= 21'sd1048575;
                    2: x_in <= -21'sd1048576;
                    3: x_in <= 21'sd1;
                    4: x_in <= -21'sd1;
                    5: x_in <= 21'sd127;
                    6: x_in <= 21'sd128;
                    7: x_in <= -21'sd128;
                    8: x_in <= -21'sd129;
                    default: x_in <= $random(seed);
                endcase
                x_in_valid <= 1'b1;
                inputs_sent <= inputs_sent + 1;
                outputs_since_input <= 0;
            end

            // The first 600 input frames use the real board condition:
            // ce128_out is asserted on every system clock.  The remainder
            // adds random inactive intervals to cover clock-enable stalls.
            if (inputs_sent < 600) begin
                ce_out <= 1'b1;
            end
            else if (ce_gap == 0) begin
                ce_out <= 1'b1;
                ce_gap <= $random(seed) & 3;
            end
            else begin
                ce_out <= 1'b0;
                ce_gap <= ce_gap - 1;
            end
        end
    end

    initial begin
        repeat (20) @(posedge clk);
        @(negedge clk);
        rst_n = 1'b1;

        wait (outputs_checked >= 4096);
        @(negedge clk);
        rst_n = 1'b0;
        repeat (4) @(posedge clk);
        @(negedge clk);
        rst_n = 1'b1;
        reset_completed = 1'b1;

        wait (inputs_sent >= 1000 && outputs_since_input >= 16);
        repeat (40) @(posedge clk);
        if (!reset_completed || mismatch_count != 0)
            $fatal(1,
                "ROUNDED STATE EQUIV FAIL outputs=%0d mismatch=%0d reset=%0b",
                outputs_checked, mismatch_count, reset_completed);
        $display("ROUNDED STATE EQUIV PASS: outputs=%0d, full-rate/random CE, directed/random data and mid-burst reset, cycle-exact 0 LSB",
                 outputs_checked);
        $finish;
    end

    initial begin
        #12000000;
        $fatal(1, "ROUNDED STATE EQUIV TIMEOUT outputs=%0d",
               outputs_checked);
    end
endmodule
