...
// 功能简述     : CIC16 N=3 前置三抽头无乘法通带补偿器。
//                差分方程为：
//
//                  y[n] = x[n-1]
//                       + (2*x[n-1] - x[n] - x[n-2]) / 8
//
//                等效 Q3 系数为 [-1, 10, -1]/8，系数严格对称，
//                直流增益为 1。乘法全部转化为移位和加减法，
//                适用于在 CIC 前修正 10～20 kHz 通带下垂。
//
//                OUTPUT_W=DATA_W 时保持兼容饱和接口；设置为
//                DATA_W+1 时保留自然峰值余量，由后级 CIC 的
//                最终量化器统一执行舍入与饱和。
//
// 当前默认配置：
//                  DATA_W=20，OUTPUT_W=DATA_W
//                  REGISTER_OUTPUT=1，输出寄存一拍
//
// 设计作者     : kafeizizi
// 创建日期     : 2026-07-29
// 版本         : V2025.2
// 开发工具     : Vivado 2025.2
// 修订记录     :
//                2026-07-29：新增三抽头移位加法 CIC 补偿器。
//                2026-08-16：补充正式工程文件头和位宽说明。
//=============================================================
//=============================================================
// 1）模块名称：cic3_compensator_shiftadd_ce
