...
//=============================================================
// 文件名       : tb_phase5_q15_single_rounder_multiseed.v
// 模块名       : tb_phase5_q15_single_rounder_multiseed
// 功能简述     : Phase 5 Q15 单舍入器多种子逐级 bit-true 测试。
//                同时驱动 V3 BRAM 参考链和 Phase 5 候选链，
//                在 Stage 2、Stage 3 和完整 128x 输出处逐拍比较。
//
//                测试覆盖：
//                  1. 24bit 最大值、最小值、零和正负小量；
//                  2. 5 组确定性 LFSR 随机满幅 PCM；
//                  3. valid=0 表示固定采样时刻输入零样点；
//                  4. 输出 valid、数据和最终样点数量一致性。
//
// 当前默认配置：
//                  输入样点数：640
//                  随机种子数：5
...
module tb_phase5_q15_single_rounder_multiseed;
        ...
        if (!rst_n) begin
            ...
            if (ce2_out) begin
                if (input_phase == 1'b0 && input_count < INPUT_COUNT) begin
                    ...
                    x_in_valid <= ((next_lfsr(lfsr) & 32'h0000001f) != 0);
                end
    ...
    initial begin
        ...
        if (ref_stage2_count == 0 || ref_stage3_count == 0 ||
            ref_full_count == 0)
            $fatal(1, "Phase 5 multiseed test produced no output");
        ...
        if (ref_full_count != dut_full_count)
            $fatal(1, "Full-chain count mismatch: ref=%0d dut=%0d",
        ...
        for (check_index = 0; check_index < ref_stage2_count;
            ...
            if (ref_stage2_stream[check_index] !==
                ...
                $fatal(1,
                    "Stage 2 stream mismatch index=%0d ref=%0d dut=%0d",
        ...
        for (check_index = 0; check_index < ref_stage3_count;
            ...
            if (ref_stage3_stream[check_index] !==
                ...
                $fatal(1,
                    "Stage 3 stream mismatch index=%0d ref=%0d dut=%0d",
        ...
        for (check_index = 0; check_index < ref_full_count;
            ...
            if (ref_full_stream[check_index] !== dut_full_stream[check_index])
                $fatal(1,
                    "Full stream mismatch index=%0d ref=%0d dut=%0d",
                    check_index, ref_full_stream[check_index],
                    dut_full_stream[check_index]);
        end

        $display("PASS: Phase 5 Q15 single-rounder multiseed test");
