...
//=============================================================
// 文件名       : tb_interp128_all2x_v3_strict_s1_ce.v
// 模块名       : tb_interp128_all2x_v3_strict_s1_ce
// 功能简述     : V3 strict Stage 1 完整七级 128x 链路测试平台。
...
//                输出文件：
//                  v3_strict_impulse_output.csv
//                  v3_strict_random_output.csv
//
// 当前默认配置：
//                  仿真时钟周期：10ns
//                  仿真时钟数  ：70000 拍
...
// 开发工具     : Vivado 2025.2
// 修订记录     :
//                2026-07-11：新增 V3 完整七级对拍测试平台。
//=============================================================
//=============================================================
// 1）模块名称：tb_interp128_all2x_v3_strict_s1_ce
// 功能说明：仿真测试平台：产生激励、监视被测模块输出并执行自动判定。
// 工程版本：Vivado 2025.2。
//=============================================================

module tb_interp128_all2x_v3_strict_s1_ce;
    ...
    // 例化说明：调用 interp128_all2x_v3_strict_s1_top_ce 插值链顶层，完成所选倍率的数据率提升与有效信号传递。
    interp128_all2x_v3_strict_s1_top_ce dut_impulse (
        .clk(clk), .rst_n(rst_n),
    ...
    // 例化说明：调用 interp128_all2x_v3_strict_s1_top_ce 插值链顶层，完成所选倍率的数据率提升与有效信号传递。
    interp128_all2x_v3_strict_s1_top_ce dut_random (
        .clk(clk), .rst_n(rst_n),
        .ce2_out(ce2_out), .ce4_out(ce4_out), .ce8_out(ce8_out),
        .ce16_out(ce16_out), .ce32_out(ce32_out),
        .ce64_out(ce64_out), .ce128_out(ce128_out),
    ...
    initial begin
        ...
        fp_impulse = $fopen("v3_strict_impulse_output.csv", "w");
        fp_random = $fopen("v3_strict_random_output.csv", "w");
        ...
        $display("V3 strict full-chain counts: impulse=%0d random=%0d",
