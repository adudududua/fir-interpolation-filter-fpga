...
// 功能简述     : N=3 串行 comb CIC 与 Hold16 等价重写的逐拍回归。
//                测试同时实例化传统串行 comb、4-DSP、3-DSP 与
//                2-DSP 映射，在相同复位、使能和输入激励下比较
//                输出有效顺序及每个有符号样本，判定标准为 0 LSB。
//
//                激励覆盖多组固定随机种子、不同输入相位和完整
//                16 倍插值突发，以确认代数降阶未改变模运算结果。
//
// 当前默认配置：DATA_W=21，OUTPUT_W=20，R=16，N=3
// 设计作者     : kafeizizi
// 创建日期     : 2026-07-29
// 版本         : V2025.2
// 开发工具     : Vivado 2025.2
// 修订记录     :
//                2026-07-29：新增 N3 Hold16 多实现等价回归。
//                2026-08-16：统一中文文件头及判定条件说明。
//=============================================================
...
module tb_cic_interp16_n3_hold_equiv;
    ...
    initial begin
        ...
        // 显式DSP48E1验证使用UNISIM glbl模型。复位保持时间超过其约100 ns
        // 配置GSR区间，避免首个梳状输入样本被器件启动复位丢弃。
        repeat (12) @(negedge clk);
        rst_n = 1'b1;

        // 在保持/突发输出仍处于活动状态时插入复位，验证中途复位恢复边界。
        stream_inputs(25, 0);
        @(negedge clk);
        x_in_valid = 1'b0;
        ce_out = 1'b1;
        ...
        // 使用10组独立随机种子，相对原33位实现同时覆盖CE停顿及有符号
        // 正、负满量程端点。
