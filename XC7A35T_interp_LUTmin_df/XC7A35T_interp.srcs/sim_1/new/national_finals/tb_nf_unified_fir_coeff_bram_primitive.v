...
// 功能简述     : 统一 FIR 系数 RAMB18E1 原语初始化与地址映射测试平台。
//                逐地址读取 Stage1 和 Stage2/3 系数，并与测试平台内的
//                签核常量逐项比较，验证位流使用的 INIT 内容、对称地址
//                折叠及不同系数位宽在器件原语中的映射均无偏移或符号错误。
// 当前默认配置 : 测试回归定义 SYNTHESIS，使 DUT 进入真实 RAMB18E1 分支；
//                Stage1 系数16位，Stage2/3 系数18位。
// 设计作者     : kafeizizi
// 创建日期     : 2026-08-16
// 版本         : V2025.2
// 开发工具     : Vivado 2025.2
...
module tb_nf_unified_fir_coeff_bram_primitive;
    ...
    initial begin
        // glbl会将器件原语模型保持在全局复位约100 ns，等待其释放后再读取。
        #120;

        // 遍历全部地址，包括布局中有意保留并填零的空洞地址。
