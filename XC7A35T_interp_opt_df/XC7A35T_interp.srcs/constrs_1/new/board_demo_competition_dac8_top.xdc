...
#                2026-07-18：按 V1.20 IO 分配表增加 12864T 液晶
#                            8bit 并口及 RS/RW/E 约束。
#                2026-07-19：增加5.645MHz音频域与100MHz独立测频域
#                            的异步时钟组约束。
# 其他描述     :
...
#                5. SW1～SW4 和 SW5～SW8 均映射为
#                   44.1kHz 家族 1x/4x/8x/128x 模式选择。
#                6. V1.20 IO 表确认 12864T：
#                   D0~D7 = P16/R16/P15/R14/P14/V7/W7/Y8；
#                   RS/RW/E = V8/T5/U7。
#=============================================================
...
#=============================================================
# 3）12864T 8bit 并行液晶约束
#
# PSB 已由底板接到 3.3V，液晶固定工作于并行模式；RST 接
# SYS_RST，背光由底板 LCD_EN 跳线控制，因此 FPGA 只需要这里
# 的 8bit 数据和 RS/RW/E 共 11 根信号。
#=============================================================
set_property PACKAGE_PIN P16 [get_ports {lcd_data[0]}]
set_property PACKAGE_PIN R16 [get_ports {lcd_data[1]}]
set_property PACKAGE_PIN P15 [get_ports {lcd_data[2]}]
set_property PACKAGE_PIN R14 [get_ports {lcd_data[3]}]
set_property PACKAGE_PIN P14 [get_ports {lcd_data[4]}]
set_property PACKAGE_PIN V7  [get_ports {lcd_data[5]}]
set_property PACKAGE_PIN W7  [get_ports {lcd_data[6]}]
set_property PACKAGE_PIN Y8  [get_ports {lcd_data[7]}]

set_property PACKAGE_PIN V8 [get_ports lcd_rs]
set_property PACKAGE_PIN T5 [get_ports lcd_rw]
set_property PACKAGE_PIN U7 [get_ports lcd_e]

set_property IOSTANDARD LVCMOS33 [get_ports {lcd_data[*]}]
set_property IOSTANDARD LVCMOS33 [get_ports {lcd_rs lcd_rw lcd_e}]
set_property DRIVE 8 [get_ports {lcd_data[*]}]
set_property DRIVE 8 [get_ports {lcd_rs lcd_rw lcd_e}]
set_property SLEW SLOW [get_ports {lcd_data[*]}]
set_property SLEW SLOW [get_ports {lcd_rs lcd_rw lcd_e}]
...
#=============================================================
# 9）音频域与独立100MHz测频域异步时钟组
#
# DAC_CLK经三级同步器进入100MHz测频域；100ms闸门结果在保持稳定后，
# 通过两级总线同步器和三级翻转标志同步器返回音频ILA域。因此两个域
# 之间不按普通同步路径做建立/保持分析。
#=============================================================

set_clock_groups -asynchronous \
    -group [get_clocks clk_audio_128x_44k1] \
    -group [get_clocks clk_out2_clk_wiz_audio_44k1]
