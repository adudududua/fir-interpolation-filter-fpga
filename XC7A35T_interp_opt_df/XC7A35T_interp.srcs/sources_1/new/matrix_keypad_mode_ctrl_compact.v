...
// 功能简述     : 赛方板 4x4 矩阵按键的紧凑模式与频率控制器。
//                保留演示实际使用的 SW1～SW9：
//                  SW1～SW4：1x / 4x / 8x / 128x 模式；
//                  SW5～SW8：+1kHz / -1kHz / 15kHz / 自动扫频。
//                  SW9：ILA 镜像抑制演示模式开关。
//                KC2 仅识别 KR0 对应的 SW9，SW10～SW16 忽略。
...
//                2026-07-18：增加 SW1～SW8 单次按键事件输出，SW5～SW8
//                            不再重复选择模式。
//                2026-07-19：最小扩展 KC2/KR0 的 SW9 单次事件，
//                            不恢复完整 16 键位图资源。
//=============================================================
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (scan_advance) begin
                ...
                if (!row2_sw9_found && row2_sw9_pressed)
                    row2_sw9_found <= 1'b1;
                ...
                if (scan_idx == 2'd3) begin
                    ...
                    if (scan_same_as_raw) begin
                        ...
                        else if ((debounced_valid != scan_valid_now) ||
                                 (scan_valid_now &&
                                  (debounced_code != scan_code_now))) begin
                            debounced_valid <= scan_valid_now;
                            debounced_code  <= scan_code_now;

                            if (scan_valid_now) begin
                                key_strobe <= 1'b1;
                                key_code   <= scan_code_now;

                                if (scan_code_now < 4'd4)
                                    mode_sel <= scan_code_now[1:0];
                            end
                        end
                    end
                    else begin
                        raw_valid  <= scan_valid_now;
                        raw_code   <= scan_code_now;
                        stable_cnt <= {STABLE_W{1'b0}};
                    end
                end
                else begin
